...
namespace sensor {
    ...
    void Sensor::configure (int argc, char ** argv) {
		...
		} else this-> _cfgPath = utils::join_path (ANON_DIR, "config.toml");
    ...
    void Sensor::configure (const common::utils::config::dict & config) {	
		...
		auto signalFile = sensorConfig.getOr <std::string> ("signal-path", utils::join_path (ANON_DIR, "signal"));
		...
		utils::own_file (signalFile, "anon");
		...
		cgroup::Cgroup c ("anon_api.slice");
		c.create ();
