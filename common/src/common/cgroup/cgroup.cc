...
namespace common::cgroup {
    ...
    void Cgroup::create () {
        if (!this-> exist ()) {
            ...
            if (isV2) {
                utils::create_directory (utils::join_path (mntPoint, this-> _name), true);
            }
        }
    }
    ...
    bool Cgroup::remove () {
        ...
        if (isV2) {
            if (this-> isSlice (true, mntPoint)) {
                ...
                utils::remove (utils::join_path (mntPoint, this-> _name));
            }
        }
    ...
    bool Cgroup::exist () const {
        ...
        auto mntPoint = utils::get_cgroup_mount_point (isV2);
        if (isV2) {
            if (utils::file_exists (utils::join_path (utils::join_path (mntPoint, this-> _name), "controllers"))) {
                return true;
            }
        }
    ...
    bool Cgroup::isSlice (bool isV2, const std::string & mntPoint) const {
        if (isV2) {
            ...
            if (utils::file_exists (cgroupPath)) {
                for (const auto & entry : utils::directory_iterator (cgroupPath)) {
