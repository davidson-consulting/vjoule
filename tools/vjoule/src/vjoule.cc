...
namespace tools::vjoule {
  ...
  void VJoule::create_default_config() {
    ...
    ofs << std::endl;
    ofs << "# A machine can have different GPU from different constructors" << std::endl;
    ofs << "# For example a integrated GPU, and an nvidia GPU card" << std::endl;
  ...
  void VJoule::create_configuration_if_needed() {
    ...
    if (!common::utils::file_exists(this-> _cfg_path)) {
  ...
  void VJoule::run() {
    ...
    if (c_pid == -1) {
      ...
      Exporter e(common::utils::join_path(this-> _vjoule_directory, "latest"));
      e.export_stdout();
    } else {
      // child process
