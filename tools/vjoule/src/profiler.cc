...
namespace tools::vjoule {
    ...
    Profiler::Profiler (const CommandLine & cmd)
		: _cmd (cmd), _nbIter (5)
    {
		std::string cwd = utils::current_directory ();

		this-> _vjouleDir =  utils::join_path ("/tmp/vjoule_prof/", utils::get_time_no_space());
		utils::create_directory (this-> _vjouleDir, true);
    ...
    void Profiler::createConfiguration () const {
		...
		ofs << "signal-path = \"" <<  utils::join_path(this-> _vjouleDir, "signal") << "\"" << std::endl;
    ...
    void Profiler::readConsumption (float & cpu, float & ram) const {
		...
		if (utils::file_exists (ramPath)) {
			std::ifstream ramF (ramPath);
			ramF >> ram;
			ramF.close ();
		}

		if (utils::file_exists (cpuPath)) {
